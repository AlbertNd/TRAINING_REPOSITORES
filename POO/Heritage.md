...
5. #### Autorisation d'accès aux propiétes et methodes (uniquement aux enfants)
    ...
    - ***Les mutateurs et les accesseurs doivent rester en public***
        ...
        ```
6. #### L'abstraction.
    - Supposons que nous avons des **Utilisateaur** qui sont soit **Administrateur** soit **Joueur**. et que l'idée serait de ne pas instancier la classe ***Utilisateur*** seule et par conséquent **d'imposer son héritage**.
        -   ```
                <?php
                    declare(strict_types=1);
            ...
            ```
            - **NB: Il n'est pas possible d'instacier la classe Utilisateur car elle abstrait** 
    - Supposons que l'on souhaite garantir un **usage particulier**, mais laisser **les classes enfants decider de la maniére dont le code doit fonctionner**. 
        - ***Donner une méthodes que chaque classe enfant devra avoir et si pas elle devra elle meme etre declarer abstraite***. 
            -   ```
                    <?php
                    declare(strict_types=1);
                    abstract class Utilisateur
                ...
                ```
7. #### Empecher un héritage. 
    - Pour interdire l'héritage d'une classe on la prefixe du mot clé **final**